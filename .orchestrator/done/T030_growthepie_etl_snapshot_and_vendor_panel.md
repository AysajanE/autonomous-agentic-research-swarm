...
- 2026-04-01: Operator workflow repair updated repo-wide review-bundle semantics so W1/W2 ETL tasks are judged in git by tracked code, manifests, samples, and run/review logs, while full `data/raw/` and `data/processed/` artifacts remain canonical local-only ETL surfaces. The prior `blocked` state was an operational false positive caused by the old gate treating gitignored ETL datasets as if they had to persist in every checkout. Restored the task to its existing Judge-approved `done` state.
